...
hotkeys, HTTP, MCP, React, or the filesystem layout. Everything that does is a port implementation on
the outside:
...
| Adapters | `crates/snapdown-*`, `apps/*` | Screen capture, SQLite, the Vault filesystem, the Local API, the MCP bridge, the publish client, the two React apps |
...
```mermaid
graph TD
    UI["React webview<br/>(desktop)"] --> CMD["Tauri commands"]
    CMD --> CORE["snapdown-core<br/>domain + ports"]
...
| Naming — TypeScript | `camelCase` values, `PascalCase` components and types; one file per component |
...
| Go | 1.25 |
| chi | 5.x |
| SQLite | 3.x, embedded — `rusqlite` on the desktop, `modernc.org/sqlite` in `web-api` |
| Node | 24.x, build-time only |
...
snapdown/
  crates/
    ...
    snapdown-mcp/        # the MCP Bridge binary
  apps/
    desktop/             # Tauri v2 app: Rust shell + React/Vite webview
      src/               # React + TypeScript
      src-tauri/         # commands, tray, hotkeys, the Local API
  web/
...
| --- | --- | --- |
| CAP-1 Capture | `snapdown-capture`, `apps/desktop/src-tauri` | AD-2, AD-4, AD-6 |
| CAP-2 Image reduction | `snapdown-capture` | AD-4 |
| CAP-3 Library and Editor | `snapdown-core`, `apps/desktop/src` | AD-1, AD-3 |
