...
| OQ-24 | A dispatched worker can be relied on to DRIVE the desktop UI when asked to, rather than answering from source | Three attempts on 2026-08-23 produced three different failures: the first exhausted its 1M context on accessibility trees; the second did static analysis and titled its report "real UI execution"; the third kept reading source files after an explicit correction naming that exact failure. If the answer is no, then every claim about the RUNNING product in this project rests on the owner opening it themselves - which is how BUG-4 was found, four waves late - and W1-style UI verification needs a different instrument or a different agent profile. Cost if wrong: nothing is ever verified in the product, only in jsdom | 2026-08-23 | agent, G5 |
