...
- Iteration: 2, commit `674b8f9` (no new commit this iteration — nothing to do until a dispatch reports)
...
- Stopped at: **Capacity** — same two builders from iteration 1 still running (`ListAgents` confirms
  both `running`). `validate.py --generate` re-run: same 6 known-baseline findings, nothing new.
  Re-dispatching or starting a third worktree now would risk a merge race with these two.
- Blocked: —
- Parked: —
- In flight:
