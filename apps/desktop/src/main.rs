...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // THE ROW MENU'S DESTRUCTIVE GROUP (ticket 16 of the Bundle Library spec).
    //
    // Ticket 12's Edit / Copy Markdown / Open file location rows are not in this build yet - see
    // `library.slint`'s header comment on `menu-target` - so this menu holds only Disassemble/Delete
    // today. Whichever single verb it offers is decided HERE, live, every time the menu opens: never
    // cached on the row (`BR-122`), so a Finding deleted between two menu openings changes the verb
    // the very next time.
...
mod tests {
    ...
    fn bundle_fixture(bundle_store: &SqliteBundleStore, bundle_id: &str, finding_ids: &[&str]) {
        let bundle = Bundle::new(
            bundle_id.to_string(),
    ...
    fn bundle_is_sealed_reads_live_never_a_cached_answer() {
        ...
        bundle_fixture(&bundle_store, "b1", &["f1", "f2"]);
    ...
    fn remove_bundle_row_and_folder_removes_both_cleanly() {
        let bundle_store = SqliteBundleStore::open_in_memory().expect("a bundle store");
        bundle_fixture(&bundle_store, "b-clean", &["f1"]);
    ...
    fn folder_removal_failing_after_the_row_is_gone_never_leaves_the_row_present_with_the_files_gone(
        ...
        let bundle_store = SqliteBundleStore::open_in_memory().expect("a bundle store");
        bundle_fixture(&bundle_store, "b-lock", &["f1"]);
