...
fn to_quality_budget_dto(qb: &QualityBudget) -> QualityBudgetDto {
    ...
    QualityBudgetDto {
        ...
        max_long_edge: qb.custom_pair.map(|p| p.max_long_edge).unwrap_or(resolved.max_long_edge),
        encoder_quality: qb.custom_pair.map(|p| p.encoder_quality).unwrap_or(resolved.encoder_quality),
    }
}
...
pub fn get_settings_impl(state: &AppState) -> Result<SettingsDto, String> {
    ...
    let latest_finding_size = latest_attribution.as_ref().map(|a| a.size_bytes).or_else(|| get_latest_finding_size_internal(&vault_path));
...
pub fn set_quality_budget_impl(
    ...
    let target_budget = if let Some(adv) = advanced {
        ...
        let validated = ResolvedPair::new(adv.max_long_edge, adv.encoder_quality)
            .map_err(|e| e.to_string())?;
    ...
    if let Ok(findings) = state.finding_store.list_findings() {
        if let Some(first) = findings.first() {
            ...
            return Some(LatestFindingAttributionDto {
                ...
                budget_name: first.finding.budget_name.clone().unwrap_or_else(|| "Auto".to_string()),
            });
        }
    }
    None
...
mod tests {
    ...
    #[test]
    fn set_quality_budget_atomic_and_validation() {
        let store = SqliteSettingsStore::open_in_memory().unwrap();
        let state = AppState {
            settings_store: Arc::new(store),
            finding_store: Arc::new(snapdown_store::sqlite::SqliteFindingStore::open_in_memory().unwrap()),
            bundle_store: Arc::new(snapdown_store::sqlite::SqliteBundleStore::open_in_memory().unwrap()),
            access_key_store: Arc::new(snapdown_store::sqlite::SqliteAccessKeyStore::open_in_memory().unwrap()),
            publication_store: Arc::new(snapdown_store::sqlite::SqlitePublicationStore::open_in_memory().unwrap()),
        ...
        // Custom valid
        let custom_pair = ResolvedPair { max_long_edge: 2000, encoder_quality: 85 };
        let res_custom = set_quality_budget_impl(NamedBudget::Auto, Some(custom_pair), &state).unwrap();
        ...
        let invalid_pair = ResolvedPair { max_long_edge: 100, encoder_quality: 85 };
