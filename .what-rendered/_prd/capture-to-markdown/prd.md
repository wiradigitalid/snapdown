...
The Reviewer can select several Bundles at once, from the reclaim-space surface, and remove every selected one — its row and its own copied images — in a single confirmed act, the bulk form of FR-14. The same act also discards the selected Bundles' own source Findings and can affect a Publication, so both are triggered here rather than reauthorised — `bundle` owns only [Bundle, BundleItem].

**Proof of done:** Selecting several Bundles in reclaim space and confirming "Delete both" removes every selected Bundle's row and its own image folder, discards the same Bundles' source Findings, and the one confirmation names the whole selected set plus any other Bundle BR-122 says will also be sealed.  
**Capability:** `CAP-4`  
**Component:** `bundle`
