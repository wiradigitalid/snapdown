...
4. **Implement / Update Tests in `crates/snapdown-store/tests/test_image_reduction.rs`:**
   ...
   - Implement the 5 required tests from `waves.yaml`:
     1. `cargo::a_reduced_image_decodes_and_its_pixels_are_the_scaled_source` â€” Assert that reduced PNG bytes decode back to `DynamicImage` and the pixels match the downscaled source patterns.
     2. `cargo::a_reduced_image_honours_the_resolved_long_edge` â€” Assert that a 3840x2160 source reduced with a 1600px max long edge produces a decoded image with long edge equal to 1600.
     3. `cargo::an_image_already_under_the_long_edge_is_not_upscaled` â€” Assert that a 400x300 source reduced with a 1600px max long edge stays 400x300.
     4. `cargo::a_thumbnail_decodes_and_is_smaller_than_its_source` â€” Assert that thumbnail bytes decode as valid PNG and dimensions are strictly smaller than original and target.
     5. `cargo::the_resolved_pair_arithmetic_is_unchanged_by_this_story` â€” Assert that pure dimension arithmetic functions (`compute_reduced_dimensions_for_pair`, `compute_reduced_dimensions_with_edge`, presets, `Auto` resolution) produce identical results.
...
- `crates/snapdown-store/Cargo.toml` â€” Declare dependency on workspace `image` crate.
- `crates/snapdown-store/src/image/pipeline.rs` â€” Implement real decode-scale-encode and thumbnail generation in `ImageReducer`.
- `crates/snapdown-store/tests/test_image_reduction.rs` â€” Comprehensive test suite implementing the 5 required tests from `waves.yaml` and updating existing reduction tests.
- `apps/desktop/src-tauri/src/commands/capture.rs` â€” Wire `ImageReducer::reduce_image` into `capture_screen_region` so real captured images are reduced before being saved to the Vault.

## Tasks & Acceptance

**Execution:**
1. `crates/snapdown-store/Cargo.toml` â€” Add `image = { workspace = true }`.
2. `crates/snapdown-store/src/image/pipeline.rs` â€” Replace fake 17-byte header and payload simulation with `image::load_from_memory`, aspect-ratio-preserving downscaling via `image::imageops::resize`, and real PNG encoding via `PngEncoder`.
3. `apps/desktop/src-tauri/src/commands/capture.rs` â€” Connect `ImageReducer::reduce_image` into `capture_screen_region` before writing blob to Vault.
4. `crates/snapdown-store/tests/test_image_reduction.rs` â€” Implement programmatic test image synthesiser and the 5 tests declared in `waves.yaml`:
...
- `cargo test -p snapdown-store --test test_image_reduction` â€” All 5 image reduction tests pass.
