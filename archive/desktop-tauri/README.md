...
| `src/components/AgentAccessView.tsx`, `src/services/agent_access.ts`, `src-tauri/src/commands/agent_access.rs`, both matching test files | The Agent access surface (issue/revoke a key) | Building the Agent access screen (`UC-17`, `UC-19`, and the surface `BUG-59`'s fix needs to create a key from) |
| `src/components/FindingsView.tsx`, `src/test/findings_view.test.tsx` | Findings list/management and its delete confirmation | `BUG-61`'s Findings list, delete (`UC-7`) |
| `src/components/OrphanReportView.tsx`, `src/test/orphan_report.test.tsx` | The orphan report | `BUG-61` / `UC-8` |
| `src/components/BundleView.tsx`, `src/services/bundle.ts`, `src-tauri/src/commands/bundle.rs`, both matching test files | Bundles list and detail | `BUG-61`'s Bundles list/detail (`UC-10`, `UC-11`) |
...
| `src/types/finding.ts`, `src/types/settings.ts` | The shapes the kept components/services use | Context only |
